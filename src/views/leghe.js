...
export const leghe = {
  ...
  mount(root, ctx) {
    ...
    root.querySelector('main').addEventListener('click', async (e) => {
      ...
      if (e.target.closest('#go-create')) { const name = root.querySelector('#lname').value.trim(); const t = team(); if (!name) { ctx.toast('Dai un nome alla lega'); return; } if (!t) return; if (!await salvaNome()) return;
      ...
      if (e.target.closest('#go-join')) { const code = root.querySelector('#code').value.trim(); const t = team(); if (code.length < 4) { ctx.toast('Inserisci il codice invito'); return; } if (!t) return; if (!await salvaNome()) return;

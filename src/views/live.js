...
function fieldTeam(res, ratings, evs, mirrored) {
  ...
  return order.map((r) => `<div class="line">${byRole[r].map((row) => {
    ...
    const ind = [];
    if (row.subFor) ind.push('<i class="up">↑</i>'); if (row.official) ind.push('<i class="dn">↓</i>');
...
function benchCol(res, ratings) {
  ...
  return res.lineup.bench.map((id) => { const r = ratings.get(id); const p = P(id);
    ...
    return `<div class="b2">${roleChip(p.role)}<span class="nm"><b>${esc(p.lastName)}${inUse.has(id) ? ' <span class="up">↑</span>' : ''}</b><span>${esc(club(id).name)}</span></span>${v}</div>`; }).join('');
}

...
export const impostazioni = {
  ...
  render() {
    ...
    const account = remote
      ...
      : `<div class="a-card"><label class="lbl">Account</label><p class="small muted">Sei in modalità locale (demo). ${S.supabaseConfigured() ? 'Supabase è configurato: puoi accedere con la tua e-mail.' : 'Per account e leghe multiple collega un progetto Supabase.'}</p>${S.connectionError() ? `<div class="warn block" style="margin-top:10px">${icon('warn', 'ic sm')}<span>${esc(S.connectionError().message)}</span></div><button class="a-btn" id="retry" style="margin-top:10px">Riprova la connessione</button>` : S.supabaseConfigured() ? `<button class="a-btn" id="go-remote" style="margin-top:10px">Accedi con e-mail</button>` : ''}</div>`;
  ...
  mount(root, ctx) {
    root.querySelector('main').addEventListener('click', async (e) => {
      ...
      if (e.target.closest('#retry')) { ctx.toast('Riprovo…'); const ok = await S.retryRemote(); ctx.toast(ok ? 'Connesso' : S.connectionError().message); if (ok) ctx.go('login'); return; }

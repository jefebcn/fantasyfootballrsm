...
alter table public.league_members add column if not exists vice_user_id uuid references public.profiles(id) on delete set null;
...
language sql stable security definer set search_path = public as $$
  select exists (
    ...
    where league_id = lid and (user_id = auth.uid() or vice_user_id = auth.uid())
  )
$$;
...
create policy members_update on public.league_members for update to authenticated
  using (public.is_league_admin(league_id) or user_id = auth.uid() or vice_user_id = auth.uid())
  with check (
    public.is_league_admin(league_id)
    or (user_id = auth.uid() and role = (select role from public.league_members m where m.id = league_members.id))
    or (vice_user_id = auth.uid()
...
declare codice text;
begin
  if not exists (select 1 from public.league_members where id = p_member and user_id = auth.uid()) then
...
begin
  ...
  if riga.user_id = auth.uid() then raise exception 'Questa squadra è già tua'; end if;
  ...
  if exists (select 1 from public.league_members
             where league_id = riga.league_id and (user_id = auth.uid() or vice_user_id = auth.uid())) then
    raise exception 'Sei già in questa lega con una squadra tua';
  end if;
  -- Il codice si consuma: il link vale una volta sola.
  update public.league_members set vice_user_id = auth.uid(), vice_code = null where id = riga.id;
...
begin
  if not exists (select 1 from public.league_members
                 where id = p_member and (user_id = auth.uid() or vice_user_id = auth.uid())) then

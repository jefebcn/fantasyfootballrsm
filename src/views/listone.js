...
export const listone = {
  ...
  render() {
    ...
    return `<main class="a-body">
      <input class="field-input" id="q" placeholder="Cerca giocatore" value="${esc(q)}" autocomplete="off">
      <div class="chips"><button class="chip${!role ? ' on' : ''}" data-role="">Tutti</button>${['P', 'D', 'C', 'A'].map((r) => `<button class="chip${role === r ? ' on' : ''}" data-role="${r}">${r}</button>`).join('')}<select id="club" class="chip" style="appearance:auto"><option value="">Società</option>${S.base.clubs.map((c) => `<option value="${c.id}" ${clubId === c.id ? 'selected' : ''}>${esc(c.name)}</option>`).join('')}</select></div>

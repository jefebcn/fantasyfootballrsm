/**
 * Tesserati presenti nei tabellini FSGC ma assenti dalle rose di
 * Transfermarkt, la cui copertura sammarinese è parziale.
 *
 * GENERATO da scripts/importa-fsgc.py — non modificare a mano.
 * Senza di loro un gol vero resterebbe senza marcatore.
 * Quotazione: la minima del ruolo, non avendo un valore di mercato.
 *
 * Forma: [clubId, nome, cognome, ruolo]
 */
export const LISTONE_EXTRA = [
  ["cailungo","Adolfo Jose","Hirsch","C"],
  ["faetano","Cristiano","Abbadessa","C"],
  ["trepenne","Danilo Ezequiel","Rinaldi","C"],
  ["academy","Gabriele","Montali","C"],
  ["academy","Marco","Zavoli","C"],
  ["trepenne","Mario Meddi","Cordioli","C"],
  ["trepenne","Mattia Luciano","Cenni","C"],
  ["cosmos","Richard Ulloa","Brito","C"],
  ["sangiovanni","Thomas","Caverzan","C"],
  ["trefiori","Tjakko Van","Nek","C"],
  ["trefiori","Bernardi Tommaso","Leon","A"],
  ["sangiovanni","Caverzan","Thomas","C"],
  ["lafiorita","Cicarelli De Morais","Daniel","C"],
  ["trepenne","Cordioli Mario","Meddi","C"],
  ["cailungo","Filippo Tordella","Manuel","P"],
  ["academy","Grandoni","Giacomo","C"],
  ["academy","Molinari","Nicola","C"],
  ["academy","Montali","Gabriele","D"],
  ["academy","Mularoni","Alessandro","C"],
  ["libertas","Osayande Aigbogun","Osagie","A"],
  ["cailungo","Parisi Maurizio","Ciro","D"],
  ["virtus","Piscitella","Gianmario","A"],
  ["murata","Raimondi","Andrea","C"],
  ["trepenne","Rinaldi Danilo","Ezequiel","C"],
  ["cosmos","Ulloa Brito","Richard","C"],
];

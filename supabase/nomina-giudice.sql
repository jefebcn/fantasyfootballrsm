...
--
-- L'id va confrontato come testo: dopo migrations/001 profiles.id è text
-- (con un fornitore d'identità esterno non è un uuid), mentre auth.users.id
-- resta uuid. Senza il cast questo file si fermava su
-- "operator does not exist: text = uuid" e non nominava nessuno.

-- 1. Controlla di avere scritto l'indirizzo giusto: deve tornare una riga.
select id, display_name, is_judge
from public.profiles
where id = (select id::text from auth.users where lower(email) = lower('contiale2001@gmail.com'));
